...
class SignalModel(Model):
    ...
    def simulate(params: list, ppm: list):
        ...
        peak_1 = params[4] * params[2] / (1 + ((ppm - params[0] - params[1])/params[3])**2) + (1-params[4]) * \
            params[2]*np.exp(-(ppm - params[0] - params[1])**2/(2*params[3]**2))
        peak_2 = params[4] * 2 * params[2] / (1 + ((ppm - params[0])/params[3])**2) + (1-params[4])*2*params[2]*np.exp(-(ppm - params[0])**2/(2*params[3]**2))
        peak_3 = params[4] * params[2] / (1 + ((ppm - params[0] + params[1])/params[3])**2) + (1-params[4]) * \
            params[2]*np.exp(-(ppm - params[0] + params[1])**2/(2*params[3]**2))
